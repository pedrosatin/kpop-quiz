...
export interface QuizSession {
  schema_version: "kpop-quiz-session-v1";
...
const QUESTION_FIELDS = ["id", "logical_id", "semantic_id", "fact_base_ids", "language", "type", "theme", "difficulty", "group_ids", "prompt", "options", "answer_option_id", "explanation", "reference_date", "evidence"];
...
function isQuestion(value: unknown, locale: Locale): value is QuizQuestion {
  ...
  if (typeof value.id !== "string" || !HASH.test(value.id)
    ...
    || !isStringArray(value.group_ids)) return false;
...
export function isQuizSession(value: unknown): value is QuizSession {
  ...
  const { config } = value;
  if (value.schema_version !== "kpop-quiz-session-v1"
    ...
    || !(config.difficulty === null || typeof config.difficulty === "string" && DIFFICULTIES.has(config.difficulty as Difficulty))
