...
In the grid the `.game-actions` bar sits under the board and holds a two-line message next to the counters ("9 palpites restantes" / "9 guesses left" and "0 de 9 casas certas" / "0 of 9 squares correct"). The message is the live region: the instructions at first, then the verdict of the last guess with the group's name ("Certa: TWICE." / "Correct: TWICE.", "Errada: SHINee." / "Wrong: SHINee."), followed by the counters for screen readers. The board never moves, and the headers, the nine cells and the bar fit at 390x844 and 1440x900. The group picker is a modal dialog: Tab stays inside it, Escape and Close return focus to the cell that opened it, and on phones it follows the visual viewport so the search field and about five groups stay above the keyboard. After a guess focus goes to the same cell when it is still open, or to the next empty cell after a right guess. A solved cell stays focusable with `aria-disabled`, so the arrow keys still walk through it, and for 300ms after a guess the board ignores taps, so a double tap on a group does not open the cell under it. The board is saved in `localStorage` under `kpop-grid-<grid_id>` after each guess; a save that names a group the cell does not accept is dropped. When the game ends the result replaces the counters in the bar: the title (described by the "8 de 9 casas certas com 9 palpites" / "8 of 9 squares correct in 9 guesses" summary), Share, Play again and a "Ver fonte" / "Show source" toggle (labelled "Ocultar fonte" / "Hide source" while open), with the high-contrast option under them. On phones the category tags hide and the rows drop to 44px so the finished board stays above the taller bar. The panel lists, for each cell, the player's answer, the accepted groups and one line per group and cited revision with every location in the source ("Local na fonte" / "Location in source"); while it is open the bar stops being sticky. Share opens the system share sheet where there is one and otherwise copies the text; if neither works, the live region says so and the text shows in a read-only field to copy by hand. The result buttons ignore activation for 300ms and held keys. Focus moves to the result title only when the game ends during the visit; a finished game restored from storage keeps focus where the page put it and is not counted again in the statistics.
