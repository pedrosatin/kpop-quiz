...
The static interface uses Astro and Preact. The build produces the `/pt-br/` and `/en/` routes at the site root. The site is published on Cloudflare Pages at <https://kpopquiz.online> (with `kpopquiz.pages.dev` as the platform URL). Pushes to the `dev` branch publish a staging build to GitHub Pages at <https://pedrosatin.github.io/kpop-quiz/>; that build sets `ASTRO_SITE` and `ASTRO_BASE` to serve the site from the `/kpop-quiz/` subdirectory. The browser validates the manifest, the SHA-256, and the session contract before starting the quiz.
...
npm test
npm run build
ASTRO_SITE=https://pedrosatin.github.io ASTRO_BASE=/kpop-quiz npm run build  # staging layout
```
