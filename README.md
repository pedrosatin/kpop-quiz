...
In the name guess game the on-screen keyboard is the `.game-actions` bar. The board's tiles shrink with the viewport height (down to 1.75rem) so the whole keyboard, ENTER and DEL included, fits under the board at 390x844 and 1440x900. A rejected guess is announced from the bar's live region and floats just above the keyboard without moving the board. When the game ends the result replaces the keyboard in the bar: the verdict, the answer, Share, Play again and a "Detalhes" / "Details" toggle for the clues and the Wikidata source. Focus moves to the result title only when the game ends during the visit, not when a finished game is restored. The result buttons ignore clicks for 300 ms and a held Enter, so a double tap on ENTER does not share or restart. The HUD keeps the guesses counter after the end, and the high-contrast toggle sits below the card; its setting is still saved with the day's game in `localStorage`.
