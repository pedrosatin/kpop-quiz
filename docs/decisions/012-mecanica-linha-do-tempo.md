...
A história do K-pop é marcada por gerações com períodos delimitados na historiografia especializada, estreias históricas de grupos, mudanças de formação e lançamentos emblemáticos. A base SQLite auditada desta fatia expõe os predicados `formed_on` e `born_on`. O predicado `debuted_on` ainda não entra na geração.
...
4. O jogador organiza a sequência com drag-and-drop ou com botões de teclado ("Mover para cima", "Mover para baixo").
...
6. Ao concluir a partida (por sucesso ou esgotamento de tentativas):
   - Cada evento revela a data documentada na precisão em que ela está armazenada (`YYYY`, `YYYY-MM` ou `YYYY-MM-DD`).
...
2. Distância temporal mínima: o intervalo entre dois eventos consecutivos na ordem cronológica é de no mínimo 30 dias. Para datas com precisão parcial, o intervalo usa os limites inferior e superior do período: um evento só com o ano ocupa de 1º de janeiro a 31 de dezembro daquele ano.
...
Nesta primeira fase, o gerador produz eventos dos tipos `formation` e `birth`, a partir dos predicados auditados `formed_on` e `born_on`. Os tipos `debut`, `member_join` e `disbandment` permanecem no contrato do esquema para uso futuro, à medida que os predicados correspondentes entrem na base auditada.
...
- `events`: lista contendo de 4 a 6 itens (padrão diário: 5) dispostos em ordem cronológica estrita, cada um com:
  ...
  * `date`: string de data ISO 8601 (`YYYY-MM-DD`, `YYYY-MM` ou `YYYY`).
  ...
  * `entity_id`: identificador Wikidata da entidade principal (ex.: `"Q21480414"` para TWICE).
...
2. **Contraste de cor e modo de alto contraste**: Todos os estados de validação (correto, incorreto, selecionado, em foco) atendem à taxa mínima de contraste de 4.5:1 em texto e 3:1 em componentes gráficos. Os estados usam ícones semânticos (✓, ✕, ⇅) além da cor.
...
### Positivas
- Adiciona uma sexta mecânica de jogo, focada em conhecimento temporal e histórico do K-pop.
...
- O contrato de dados deixa espaço para um cliente WCAG 2.2 AA. Esta fatia não entrega a interface da linha do tempo.
