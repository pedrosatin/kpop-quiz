...
Runner unificado e workflow `daily-puzzles-cron.yml`: reconstrói o banco com cache incremental, gera os seis jogos diários (`grid`, `connections`, `name-guess`, `word-search`, `timeline` e as sessões diárias nos dois idiomas), valida cada artefato contra seu schema em diretório temporário e publica por troca atômica somente quando todas as verificações passam. Se o rebuild falhar 3 vezes, com 10 minutos entre as tentativas, o workflow restaura o banco do cache e publica os jogos do dia a partir dele. Quando o grid do dia não fecha um 3x3, o runner mantém o anterior e o workflow mostra um aviso no resumo da execução. A semente de cada jogo deriva da data (`kpop-{jogo}-daily-{AAAA-MM-DD}`).
